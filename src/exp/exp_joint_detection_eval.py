...
from data.dataloader import create_non_graph_loader,create_non_graph_ssl_loader
import utils
...
class Exp_JOINT_DETECTION_EVAL(Exp_Basic):
    def __init__(self, args):
        super(Exp_JOINT_DETECTION_EVAL, self).__init__(args)
    ...
    def _get_downstream_dataloader(self,args,split,shuffle):
        return create_non_graph_loader(args,split,shuffle,"Detection")
    ...
    def cluster_loss(self, embedding, train_labels,update_average = False):
        ...
        embedding_dict = torch.zeros(2, self.args.hidden_dim).to(self.device)
        # print(embedding_dict.shape,embedding.shape)
        for label_id in range(2):
            index_i = [i for i, item in enumerate(train_labels) if int(item) == label_id]
            if len(index_i) == 0:
                continue
        ...
        embedding_targets = embedding_targets.reshape(embedding.shape)
    ...
    def train_epoch_batched(self, model, optimizer, train_loader, epoch):
        ...
        for batch_X, batch_y, x_labels in tqdm(train_loader, total=len(train_loader)):
            ...
            loss_class = self.criterion(predict_class, train_labels.to(self.device))
            if epoch >= self.args.drop_task_epoch:
                ...
                loss_batch = loss_batch*self.args.cluster_prediction_weight + cluster_loss* self.args.cluster_attract_weight + diverge_loss*self.args.cluster_repel_weight
    ...
    def valid_batch(self, model, valid_loader):
        ...
        return results_list[4][1],loss_mean
    ...
    def train(self, settings):
        ...
        for epoch in range(1,self.args.n_epochs):
            ...
            # self.pprint("average: ", self.embedding_average)
