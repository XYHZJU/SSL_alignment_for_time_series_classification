...
class Exp_Basic(object):
    ...
    def plot_cluster_loss(self, cluster, diverge, path, args):
        ...
        np.save(path+'npys/diverge_loss.npy',np.array(diverge))
        
        p1 = plt.plot(x, cluster,'r-', label = u'cluster')
        plt.legend()
        p2 = plt.plot(x, diverge, 'b-', label = u'diverge')
        plt.legend()
        plt.xlabel(u'epoches')
        plt.ylabel(u'loss')
    ...
    def plot_SSL_metrics(self, LogME, SFDA, NLEEP, path, args):
        ...
        plt.legend()
        plt.xlabel(u'epoches')
        plt.ylabel(u'SSL_metrics')
        # plt.ylim(-0.25,1.75)
        plt.title('SSL_metrics, lr: '+str(learning_rate)+" decay: "+str(decay))
